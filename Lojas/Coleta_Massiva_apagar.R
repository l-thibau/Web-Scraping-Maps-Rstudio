...
dados_lojas_feira <- pegar_dados(local = "Feira de Santana", termo = "Loja de presentes", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_de presentes.xlsx")
...
# Chamar a função para coletar dados
dados_lojas_feira <- pegar_dados(local = "Feira de Santana", termo = "Atacadista", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_Atacadista.xlsx")
...
# Chamar a função para coletar dados
dados_lojas_feira <- pegar_dados(local = "Feira de Santana", termo = "Pet Shop", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_Pet Shop.xlsx")
...
# Chamar a função para coletar dados
dados_lojas_feira <- pegar_dados(local = "Feira de Santana", termo = "loja de suprimentos para animais de estimação", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_suprimentosparaanimaisdeestimacao.xlsx")
...
# Chamar a função para coletar dados
dados_lojas_feira <- pegar_dados(local = "Salvador", termo = "loja de materiais de construção", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_materiaisdeconstrucao.xlsx")
...
# Chamar a função para coletar dados
dados_lojas_feira <- pegar_dados(local = "Petrolina ", termo = "loja de artigos domésticos", scrolls = 2)
...
# Exportar para Excel
write_xlsx(dados_lojas_feira, "loc_dados_lojas_artigosdomésticos.xlsx")

# Função para remover duplicatas considerando múltiplas colunas
remover_duplicatas <- function(dados, colunas) {
  dados %>%
    group_by(across(all_of(colunas))) %>%
    filter(row_number() == 1) %>%
...
temas <- c("Loja de materiais de construção", "Loja de variedades", "Loja de utensílios de cozinha", "Atacadista de utilidades domésticas", 
           "Loja de Departamento", "Loja de presentes", "Atacadista", "Pet Shop", 
           "Loja de suprimentos para animais de estimação")

nomes_arquivos <- c("loc_dados_lojas_materiaisdeconstr_ssa.xlsx", "loc_dados_lojas_variedades_ssa.xlsx", "loc_dados_lojas_utensilios_cozinha_ssa.xlsx", 
...
# Loop para processar cada tema
for (i in seq_along(temas)) {
  cat("\n\nProcessando tema:", temas[i], "\n")
  
  # Coletar dados para o tema atual
  dados_lojas <- pegar_dados(local = "Petrolina", termo = temas[i], scrolls = 2)
  
  # Remover duplicatas
  dados_lojas <- remover_duplicatas(dados_lojas, colunas_para_verificar)
  
  # Exportar para Excel
  write_xlsx(dados_lojas, nomes_arquivos[i])
  
  cat("\n\nDados exportados para:", nomes_arquivos[i], "\n")
}

cat("\n\nTodos os temas foram processados e os dados exportados com sucesso! 💕\n")
