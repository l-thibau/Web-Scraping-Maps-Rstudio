...
coletar_informacoes <- function(remDr) {
  ...
  if (is.na(endereco)) {
    ...
    # Indicando erro da primeira tentativa
    cat("\n❌ Endereço não identificado na primeira tentativa. \n")
    ...
    endereco <- tryCatch({
      ...
      if (!is.na(endereco)) {cat("\n✅ Endereço coletado com sucesso na segunda tentativa. \n")}

    }, error = function(e) {

      # Indicando erro da segunda tentativa
...
pegar_dados <- function(local = "", termo = "", scrolls = 0, num_while = 5) {

  # scroll tá fazendo tudo menos "scrollando" como deveria, eu passei o parâmetro 2 pra fazer uma busca curta, e segue indo até o talo
